...
public class SortedNeighborhood {
    ...
    public Set<Duplicate> detectDuplicates(Relation relation, int[] sortingKeys, int windowSize, RecordComparator recordComparator) {
        ...
        for (int sortingKey : sortingKeys) {
            ...
            Arrays.sort(records, Comparator.comparing(r -> r.values[sortingKey]));

            // Apply windowing and compare records within the window
            for (int i = 0; i < records.length - windowSize + 1; i++) {
                for (int j = i + 1; j < i + windowSize && j < records.length; j++) {
                    ...
                    if (recordComparator.isDuplicate(similarity)) {
                        duplicates.add(new Duplicate(records[i].index, records[j].index));
                    }
                }
            }
        }
    ...
    public static RecordComparator suggestRecordComparatorFor(Relation relation) {
        ...
        for (int i = 0; i < relation.getAttributes().length; i++) {
            ...
            // Example heuristics to define similarity measures and weights
            if (attributeName.equalsIgnoreCase("name") || attributeName.equalsIgnoreCase("title")) {
                measure = new Levenshtein();
                weight = 0.3;  // Higher weight for names or titles
            } else if (attributeName.equalsIgnoreCase("description") || attributeName.equalsIgnoreCase("content")) {
                measure = new Jaccard(new Tokenizer());
                weight = 0.2;  // Lower weight for descriptions or content
            } else {
                measure = new Levenshtein();
                weight = 0.1;  // Default lower weight for other attributes
            }

            attrSimWeights.add(new AttrSimWeight(i, measure, weight));
        }

        // Normalize weights (this will be handled by the RecordComparator)
        return new RecordComparator(attrSimWeights, 0.5);  // Example threshold
    }
